    ...
    public class CompSoul : ThingComp
    {
            ...
            else
            {
                ...
                if (Pawn.Faction.def.GetType() == typeof(FactionDefUniform))
                {
                    FactionDefUniform Facdef = this.Pawn.Faction.def as FactionDefUniform;
                        PawnAfflictionProps.Patron = DefDatabase<PatronDef>.GetNamed(Facdef.PreferredChaosGod);                    
                }
        ...
        public void GenerateHediffsAndImplants(ChaosFollowerPawnKindDef cdef)
        {
            if (cdef.AdditionalImplantCount.min > 0)
            {
                ...
                for (int i = 0; i < num; i++)
                {
                    PawnTechHediffsGenerator.GenerateTechHediffsFor(this.Pawn);
                }
            }
        ...
        public void DiscoverAlignment()
        {
            ...
            string letterText = "AlignmentDiscovered".Translate(new object[] { this.Pawn.Name, this.Patron.label });
        ...
        public void AlignmentDiscoveryFailure(Pawn investigator, Pawn target, float skillfactor)
        {
            ...
            Messages.Message("AlignmentDiscoveryFailed".Translate(new object[] { investigator.Name, target.Name }), target, MessageTypeDefOf.NegativeEvent);
        }
