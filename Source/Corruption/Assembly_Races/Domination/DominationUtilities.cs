...
namespace Corruption.Domination
{
    public static class DominationUtilities
    {
        public static int ActiveFightersFor(Faction faction, Map map)
        {
            ...
            return targets.FindAll(x => GenHostility.IsActiveThreatTo(x, faction)).Count;
        }
        ...
        public static void GenerateRandomBattleZone()
        {
            ...
            {
                PoliticalAlliance alliance = CFind.StoryTracker.DominationTracker.GetRandomAlliance();
        ...
        public static BattleZone GenerateBattleZoneFor(Faction aggressor, Faction defender)
        {
            BattleZone zone = (BattleZone)WorldObjectMaker.MakeWorldObject(C_WorldObjectDefOf.BattleZone);
            List<PoliticalAlliance> alliances = new List<PoliticalAlliance>();
            alliances.Add(CFind.DominationTracker.GetAllianceOfFaction(aggressor));
            alliances.Add(CFind.DominationTracker.GetAllianceOfFaction(defender));
            zone.InitializeBattle(BattleSize.Random, BattleType.OpenField, alliances, "NamerBattleGeneric");
            zone.Tile = TileFinder.RandomStartingTile();
            Find.WorldObjects.Add(zone);
            return zone;
        ...
        public static void CreateConstructionSiteAt(int tile, WorldObjectDef objectToBuild, bool informPlayer = false)
        {
            ...
            constructionSite.SetObjectToBuild(objectToBuild);
            constructionSite.Tile = tile;
            Find.WorldObjects.Add(constructionSite);
            if (informPlayer)
            {
                Find.LetterStack.ReceiveLetter("StartedBuildingWorldObject".Translate(), "StartetBuildingWorldObjectDesc".Translate(), LetterDefOf.PositiveEvent);
            }
        }
