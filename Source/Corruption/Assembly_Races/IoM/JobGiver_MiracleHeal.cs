    ...
    public class JobGiver_MiracleHeal : JobGiver_AICastPsykerPower
    {
        ...
        private bool PatientAvailableForTreatment(Pawn pawn, out Pawn patient)
        {
            Predicate<Thing> validator = delegate (Thing t)
            {
                ...
                if (pawn != null)
                    {
                    return WorkGiver_Tend.GoodLayingStatusForTend(pawn2, pawn) && HealthAIUtility.ShouldBeTendedNowByPlayer(pawn2) && pawn.CanReserve(pawn2, 1, -1, null, false) && !pawn.Faction.HostileTo(Faction.OfPlayer);
                }
                else
                {
                    return false;
